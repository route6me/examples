...
In Hermes, add this to `~/.hermes/config.yaml` (all 23 tools are discovered at startup):
```yaml
mcp_servers:
  route6:
...
Full parameter reference for all 23 tools: [references/tools.md](references/tools.md).

## Tier availability (23 tools total)
...
Rotation is instant (the address moves at the hub — no server-side reload). Typical hygiene loop: `identity { action: "check_reputation" }` → if listed, `identity { action: "set_ipv6" }` → re-check.
